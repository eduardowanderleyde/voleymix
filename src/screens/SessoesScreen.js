    ...
    <Screen edges={['bottom']}>
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={styles.subtitle}>Suas peladas de sempre, prontas pra sortear.</Text>
        </View>
      ...
      {carregando ? (
        ...
        <View style={styles.center}>
          <Feather name="calendar" size={32} color={colors.inkSoft} style={{ marginBottom: 10 }} />
          <Text style={styles.emptyText}>
            Nenhuma sessão criada ainda. Cadastre a pelada de sempre (ex: "toda terça, 19h") pra não
            precisar marcar presença do zero a cada vez.
          </Text>
          <TouchableOpacity style={styles.emptyButton} onPress={() => navigation.navigate('NovaSessao')}>
            <Text style={styles.emptyButtonText}>Criar sessão</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 12, color: colors.inkSoft, marginTop: 2, maxWidth: 200 },
  addButton: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  emptyText: { color: colors.inkSoft, textAlign: 'center', fontSize: 14, marginBottom: 16, lineHeight: 20 },
  emptyButton: {
    borderWidth: 1,
